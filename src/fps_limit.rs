...
impl<T> FpsLimit<T> {
    ...
    pub fn on_new_frame(&mut self, f: T, ts: Duration) -> Option<T> {
        ...
        let last_sent_ts = self.last_sent_ts.unwrap();
        self.on_deck = Some((ts, f));

        if ts - last_sent_ts < self.min_dt {
            // drop
